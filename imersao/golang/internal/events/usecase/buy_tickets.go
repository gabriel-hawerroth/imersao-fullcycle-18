...
type BuyTicketsUseCase struct {
	...
	patnerFactory service.PartnerFactory
}

func NewBuyTicketsUseCase(repo domain.EventRepository, patnerFactory service.PartnerFactory) *BuyTicketsUseCase {
	return &BuyTicketsUseCase{
		repo:          repo,
		patnerFactory: patnerFactory,
	}
}
...
func (uc *BuyTicketsUseCase) Execute(input BuyTicketsInputDTO) (*BuyTicketsOutputDTO, error) {
	...
	partnerService, err := uc.patnerFactory.CreatePartner(event.PartnerID)
	if err != nil {
		return nil, err
	}
	...
	for i, reservation := range reservationResponse {
		...
		ticket, err := domain.NewTicket(event, spot, domain.TicketKind(reservation.TicektKind))
		if err != nil {
			return nil, err
		}
