...
import (
	...
	"github.com/gabriel-hawerroth/imersao-fullcycle-18/golang/internal/events/domain"
	_ "github.com/lib/pq"
)
...
func (r *postgresEventRepository) FindEventByID(eventID string) (*domain.Event, error) {
	query := `
		SELECT 
			e.id, e.name, e.location, e.organization, e.rating, e.date, e.image_url, e.capacity, e.price, e.partner_id,
			s.id, s.event_id, s.name, s.status, s.ticket_id,
...
func (r *postgresEventRepository) CreateEvent(event *domain.Event) error {
	query := `
...
func (r *postgresEventRepository) FindSpotByID(spotID string) (*domain.Spot, error) {
	query := `
		SELECT
			s.id, s.event_id, s.name, s.status, s.ticket_id,
			t.id, t.event_id, t.spot_id, t.ticket_kind, t.price
...
func (r *postgresEventRepository) CreateSpot(spot *domain.Spot) error {
	query := `
...
func (r *postgresEventRepository) ReserveSpot(spotID, ticketID string) error {
	query := `
		UPDATE spots
...
func (r *postgresEventRepository) CreateTicket(ticket *domain.Ticket) error {
	query := `
...
func (r *postgresEventRepository) FindSpotsByEventID(eventID string) ([]*domain.Spot, error) {
	query := `
...
func (r *postgresEventRepository) FindSpotByName(eventID, spotName string) (*domain.Spot, error) {
	query := `
		SELECT
			s.id, s.event_id, s.name, s.status, s.ticket_id,
	...
	if ticketId.Valid {
		...
		ticket.EventID = ticketEventId.String
		ticket.Spot = &spot
		ticket.TicketKind = domain.TicketKind(ticketType.String)
		ticket.Price = ticketPrice.Float64
		spot.TicketID = ticket.ID
	}
